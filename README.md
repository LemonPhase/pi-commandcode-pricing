...
Keys inside a popup: **Tab** cycles sort (Credits → Intelligence → Value intel/$), **`/`** filters by model name (Backspace on empty cancels), `↑↓`/`jk` scroll, `←→`/`hl` pan columns when overflowing, `PgUp/PgDn`/`u d` page, `g`/`G` top/end, `Esc` clears search then closes.
